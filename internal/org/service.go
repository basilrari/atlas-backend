...
func (s *Service) CreateOrg(ctx context.Context, in CreateOrgInput, userID uuid.UUID) (*models.Org, error) {
...
func (s *Service) UpdateOrg(ctx context.Context, orgID uuid.UUID, fields map[string]interface{}) (*models.Org, error) {
	if orgID == uuid.Nil {
		return nil, errors.New("Missing org_id")
	}
	if len(fields) == 0 {
	...
	result := s.DB.WithContext(ctx).Model(&models.Org{}).
		Where("org_id = ?", orgID).
		Updates(valid)
	if result.Error != nil {
