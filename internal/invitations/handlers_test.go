...
func setupInvitationsTest(t *testing.T) (*Handlers, *Service, *gorm.DB) {
	db, err := gorm.Open(sqlite.Open(":memory:"), &gorm.Config{})
	require.NoError(t, err)
	require.NoError(t, db.AutoMigrate(&models.User{}, &models.Org{}, &models.Invitation{}))
	svc := &Service{DB: db}
	h := &Handlers{
		Service: svc,
...
func TestCheckToken_ValidToken(t *testing.T) {
	...
	orgID := uuid.New()
	require.NoError(t, db.Create(&models.Org{
		OrgID: orgID, OrgName: "TestOrg", OrgCode: "TO-123456", CountryCode: "US",
	}).Error)

	require.NoError(t, db.Create(&models.Invitation{
